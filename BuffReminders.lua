...
local function SyncSecureButtons()
    ...
    for _, frame in pairs(buffFrames) do
        ...
        local overlay = frame.clickOverlay
        if overlay then
            local clickable = frame.buffCategory and IsCategoryClickable(frame.buffCategory)
            ...
            for _, extra in ipairs(frame.extraFrames) do
                ...
                if extraOverlay then
                    if extra:IsShown() then
                        local extraClickable = frame.buffCategory and IsCategoryClickable(frame.buffCategory)
...
eventFrame:SetScript("OnEvent", function(_, event, arg1, arg2)
    ...
    elseif event == "PLAYER_SPECIALIZATION_CHANGED" then
        ...
        RefreshOverlaySpells()
        UpdateDisplay()
        -- Spells can be re-applied shortly after spec swap; refresh once more.
        C_Timer.After(0.5, function()
            if not InCombatLockdown() then
                RefreshOverlaySpells()
            end
    ...
    elseif event == "SPELLS_CHANGED" then
        -- Catch delayed spell availability after spec/talent changes.
        BR.BuffState.InvalidateSpellCache()
        RefreshOverlaySpells()
        UpdateDisplay()
