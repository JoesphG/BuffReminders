...
-- Pet summon spell IDs (hunter + other pet classes)
...
local function HunterSlotHasPet(slotIndex)
    ...
    local creatureId = info.creatureID or info.displayID or info.speciesID or info.petID
...
local function GetPetFamilyIconFromStableInfo(info)
    if not info then
        return nil
    end
    local familyId = info.familyID or info.familyId or info.petFamilyId
    if familyId and C_PetJournal and C_PetJournal.GetPetFamilyInfo then
        local _, icon = C_PetJournal.GetPetFamilyInfo(familyId)
        return icon
    end
    return nil
end
...
local function BuildPetActionItemsForBuff(buff)
    ...
    if playerClass == "HUNTER" then
        ...
        for _, entry in ipairs(HUNTER_CALL_PET_SPELLS) do
            ...
            if HunterSlotHasPet(slot) then
                local info = GetStableInfo(slot)
                local icon = nil
                local family = nil
                local familyIcon = nil
                local hoverIcon = nil
                if info then
                    icon = info.icon or info.iconFileID
                    family = GetPetFamilyFromStableInfo(info)
                    familyIcon = GetPetFamilyIconFromStableInfo(info)
                    if info.specID then
                        hoverIcon = AbilityIconForSpec(info.specID)
                    end
                end
                ...
                items[#items + 1] = {
                    ...
                    icon = icon,
                    petFamily = family,
                    petFamilyIcon = familyIcon,
                    hoverIcon = hoverIcon,
                }
            end
        end

        if KnowSpell(HUNTER_REVIVE_PET) then
