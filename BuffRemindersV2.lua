...
local function CreateActionButton()
    ...
    btn:SetScript("OnEnter", function(self)
        if self._br_hoverIcon then
            self._br_icon_restore = self.icon:GetTexture()
            self.icon:SetTexture(self._br_hoverIcon)
        end
    ...
    btn:SetScript("OnLeave", function(self)
        if self._br_icon_restore then
            self.icon:SetTexture(self._br_icon_restore)
    ...
    btn.specOverlay = btn:CreateTexture(nil, "OVERLAY")
    btn.specOverlay:SetSize(14, 14)
    btn.specOverlay:SetPoint("TOPLEFT", 1, -1)
    btn.specOverlay:Hide()
...
---@param frame table
---@param actionItems table[]?
---@param catSettings table
local function UpdatePetInlineButtons(frame, actionItems, catSettings)
    ...
    for i, item in ipairs(actionItems) do
        ...
        btn.icon:SetTexture(item.icon or 134400)
        btn._br_tooltipTitle = item.label or "Summon"
        btn._br_tooltipSub = item.subLabel
        btn._br_familyIcon = item.familyIcon
        btn._br_familyName = item.familyName
        btn._br_hoverIcon = item.hoverIcon
        if item.specAtlas then
            btn.specOverlay:SetAtlas(item.specAtlas)
            btn.specOverlay:Show()
        else
            btn.specOverlay:Hide()
        end
        btn._br_count = nil
    ...
    local spacing = math.floor(size * (catSettings.spacing or 0.2))
    local totalWidth = #actionItems * size + (#actionItems - 1) * spacing
    frame._br_action_inline = true
    frame._br_inline_size = size
    frame._br_inline_spacing = spacing
    frame._br_inline_count = #actionItems
    frame._br_inline_width = totalWidth
...
local function SyncSecureButtons()
    ...
    for _, frame in pairs(buffFrames) do
        ...
        -- Sync action buttons (consumable item row or pet inline row)
        if frame.actionButtons then
            if frame:IsShown() then
                ...
                if left then
                    ...
                    if frame._br_action_inline then
                        size = frame._br_inline_size or (catSettings.iconSize or 64)
                        btnSpacing = frame._br_inline_spacing or math.floor(size * 0.2)
                    end
                    ...
                    if visibleCount > 0 then
                        ...
                        if frame._br_action_inline then
                            maxPerRow = visibleCount
                        end
                        for _, btn in ipairs(frame.actionButtons) do
                            if btn._br_visible then
                                local col = idx % maxPerRow
                                local row = math.floor(idx / maxPerRow)
                                local btnX = left + col * (size + btnSpacing)
                                local btnY = bottom + (frame._br_action_inline and 0 or (ACTION_ICON_Y_OFFSET - size - row * (size + btnSpacing)))
                                if not frame._br_action_inline then
                                    ...
                                    btnY = bottom + ACTION_ICON_Y_OFFSET - size - row * (size + btnSpacing)
                                end
...
local function PositionFramesInContainer(container, frames, iconSize, spacing, direction)
    ...
    for i, frame in ipairs(frames) do
        ...
        if direction == "LEFT" then
            ...
            local offset = 0
            for idx = 1, i - 1 do
                offset = offset + FrameWidth(frames[idx]) + spacing
            end
        ...
        elseif direction == "UP" then
            local offset = 0
            for idx = 1, i - 1 do
                offset = offset + FrameHeight(frames[idx]) + spacing
            end
        ...
        elseif direction == "DOWN" then
            ...
            local totalWidth = 0
            for idx = 1, count do
                totalWidth = totalWidth + FrameWidth(frames[idx])
            end
            ...
            local thisWidth = FrameWidth(frame)
            frame:SetPoint("CENTER", container, "CENTER", startX + offset + thisWidth / 2, 0)
        end
    end
end
...
SetRebuffBorder = function(frame, show)
    if show then
        local color = (BuffRemindersV2DB and BuffRemindersV2DB.defaults and BuffRemindersV2DB.defaults.consumableRebuffColor)
            or { 1, 0.5, 0 }
    ...
    elseif entry.displayType == "expiring" then
        ...
        if BUFF_KEY_TO_CATEGORY[frame.key] then
            ...
            if items then
                ...
                SetExpirationGlow(frame, false)
                SetRebuffBorder(frame, false)
            elseif (BuffRemindersV2DB.defaults or {}).showConsumablesWithoutItems then
...
UpdateDisplay = function()
    ...
    local displayMode = (BuffRemindersV2DB.defaults or {}).consumableDisplayMode
...
BR.CustomBuffs = {
    ...
    UpdateFrame = function(key, spellIDValue, displayName)
        ...
        if frame then
            ...
            local customBuff = BuffRemindersV2DB and BuffRemindersV2DB.customBuffs and BuffRemindersV2DB.customBuffs[key]
            if customBuff then
                RegisterGlowBuff(customBuff, "custom")
            end
        end
...
local function SlashHandler(msg)
    ...
    elseif cmd == "lock" then
        BuffRemindersV2DB.locked = true
        HideAllMovers()
        BR.Components.RefreshAll()
        print("|cff00ccffBuffRemindersV2:|r Frames locked.")
    elseif cmd == "unlock" then
        BuffRemindersV2DB.locked = false
        UpdateAnchor()
        BR.Components.RefreshAll()
        print("|cff00ccffBuffRemindersV2:|r Frames unlocked.")
    else
        BR.Options.Toggle()
    end
end
    ...
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        if BuffRemindersV2DB.showLoginMessages ~= false then
            C_Timer.After(3, function()
                print(
                    "|cff00ccffBuffRemindersV2:|r Now supporting clickable icons for raid buffs and consumables! Enable it in |cffffcc00/brv2|r options."
                )
            end)
        end
        ...
        slashInfo:SetText("Slash commands: /brv2, /brv2 lock, /brv2 unlock, /brv2 test")
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        if not mainFrame then
            ...
            for _, cat in ipairs(CATEGORIES) do
                local cs = BuffRemindersV2DB.categorySettings and BuffRemindersV2DB.categorySettings[cat]
